...
def _animate_history(hist: npt.NDArray[np.bool_ | np.int_], normeach: bool, cmap: str | LinearSegmentedColormap):
    ...
    def animate(i: int):
        ...
        if cmap == 'gray':
            ...
            plt.imshow(
                _tile_symmetrically(_normalize(hist[i])) if normeach else hist[i], cmap=cmap, vmin=0.0, vmax=255.0
            )
...
def _tile_symmetrically(arr: npt.NDArray[np.int_]):
    ...
    return np.concatenate(
        (
            ...
            np.concatenate((np.flip(np.flip(arr, axis=1), axis=0), np.flip(arr, axis=0)), axis=1),
...
class Golident:
    ...
    def _run(self, iterations: int) -> None:
        ...
        self.history = np.empty((iterations + 1, self.board.shape[0], self.board.shape[1]), dtype=np.bool_)
