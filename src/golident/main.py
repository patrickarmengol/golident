...
def init_board(hash_length: int, seed: bytes):
    ...
    hash_bytes = seed
    hash_bits = np.empty(len(hash_bytes) * 8, dtype=bool)
...
def step(board: np.ndarray[Any, np.dtype[Any]]) -> np.ndarray[Any, np.dtype[Any]]:
...
def run(board: np.ndarray[Any, np.dtype[Any]], iterations: int):
    # NOTE: more efficient to just store the last iteration where True in a 2d array than track history
    history = np.zeros((iterations + 1, board.shape[0], board.shape[1]), dtype=bool)
    history[0] = board
...
def count_neighbors(board: np.ndarray[Any, np.dtype[Any]]) -> np.ndarray[Any, np.dtype[Any]]:
    return convolve2d(board, np.ones((3, 3)), mode="same", boundary="wrap") - board


def draw_hist(history: np.ndarray[Any, np.dtype[Any]]) -> None:
    ax = plt.figure().add_subplot(projection='3d')
    ax.voxels(history)
    plt.show()


def draw_ident(ident: np.ndarray[Any, np.dtype[Any]], cmap: LinearSegmentedColormap):
    plt.imshow(ident, cmap=cmap)
    plt.axis('off')
    plt.show()


def ident_array(history: np.ndarray[Any, np.dtype[Any]]) -> np.ndarray[Any, np.dtype[Any]]:
